...
function upcoming_3 () {
    ...
    $stmt = $pdo->query('SELECT * FROM events_table WHERE DAY(date_time) >= DAY(NOW()) AND MONTH(date_time) = MONTH(NOW()) OR DAY(date_time) < DAY(NOW()) AND MONTH(date_time) > MONTH(NOW()) ORDER BY date_time asc LIMIT 5');
